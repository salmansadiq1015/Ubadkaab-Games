...
import { Shapes, Trophy, Star, Target, ArrowLeft, Globe } from "lucide-react"
import ShapesPlayScreen from "./shapes-play"

interface ShapesMainPageProps {
  onBack: () => void
}

export default function ShapesMainPage({ onBack }: ShapesMainPageProps) {
  const { t, language, setLanguage } = useLanguage()
  const { progress } = useProgress()
  const [showPlayScreen, setShowPlayScreen] = useState(false)

  if (showPlayScreen) {
    return <ShapesPlayScreen onBack={() => setShowPlayScreen(false)} />
  }

  const gameStats = {
    highScore: progress.bestScores.shapes || 0,
        ...
        <div className="max-w-4xl mx-auto">
          <Card className="bg-white/60 backdrop-blur-sm border-white/20 mb-8">
            <CardHeader className="text-center">
              <div className="w-24 h-24 bg-gradient-to-br from-indigo-400 to-indigo-600 rounded-3xl flex items-center justify-center mx-auto mb-4">
                <Shapes className="w-12 h-12 text-white" />
              </div>
              <CardTitle className="text-3xl">{t("shapesGame")}</CardTitle>
              <CardDescription className="text-lg">{t("shapesDesc")}</CardDescription>
            </CardHeader>
            <CardContent className="text-center">
              <Button
                onClick={() => setShowPlayScreen(true)}
                size="lg"
                className="bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white px-8 py-4 text-xl"
              >
                {t("playNow")}
              </Button>
            </CardContent>
