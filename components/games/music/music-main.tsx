...
import { Music, Trophy, Star, Target, ArrowLeft, Globe } from "lucide-react"
import MusicPlayScreen from "./music-play"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"

interface MusicMainPageProps {
  onBack: () => void
}

export default function MusicMainPage({ onBack }: MusicMainPageProps) {
  const { t, language, setLanguage } = useLanguage()
  const { progress } = useProgress()
  const [showPlayScreen, setShowPlayScreen] = useState(false)

  if (showPlayScreen) {
    return <MusicPlayScreen onBack={() => setShowPlayScreen(false)} />
  }

  const gameStats = {
    highScore: progress.bestScores.music || 0,
        ...
        <div className="max-w-4xl mx-auto">
          <Card className="bg-white/60 backdrop-blur-sm border-white/20 mb-8">
            <CardHeader className="text-center">
              <div className="w-24 h-24 bg-gradient-to-br from-violet-400 to-violet-600 rounded-3xl flex items-center justify-center mx-auto mb-4">
                <Music className="w-12 h-12 text-white" />
              </div>
              <CardTitle className="text-3xl">{t("musicGame")}</CardTitle>
              <CardDescription className="text-lg">{t("musicDesc")}</CardDescription>
            </CardHeader>
            <CardContent className="text-center">
              <Button
                onClick={() => setShowPlayScreen(true)}
                size="lg"
                className="bg-gradient-to-r from-violet-500 to-purple-500 hover:from-violet-600 hover:to-purple-600 text-white px-8 py-4 text-xl"
              >
                {t("playNow")}
              </Button>
            </CardContent>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <span>Learn musical patterns</span>
                </div>
                <div className="flex items-center gap-3">
                  <Badge variant="secondary">Instruments</Badge>
                  <span>Identify different sounds</span>
                </div>
                <div className="flex items-center gap-3">
