...
import { Palette, Trophy, Star, Target, ArrowLeft, Globe } from "lucide-react"
import ColorsPlayScreen from "./colors-play"

interface ColorsMainPageProps {
  onBack: () => void
}

export default function ColorsMainPage({ onBack }: ColorsMainPageProps) {
  const { t, language, setLanguage } = useLanguage()
  const { progress } = useProgress()
  const [showPlayScreen, setShowPlayScreen] = useState(false)

  if (showPlayScreen) {
    return <ColorsPlayScreen onBack={() => setShowPlayScreen(false)} />
  }

  const gameStats = {
    highScore: progress.bestScores.colors || 0,
        ...
        <div className="max-w-4xl mx-auto">
          <Card className="bg-white/60 backdrop-blur-sm border-white/20 mb-8">
            <CardHeader className="text-center">
              <div className="w-24 h-24 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-3xl flex items-center justify-center mx-auto mb-4">
                <Palette className="w-12 h-12 text-white" />
              </div>
              <CardTitle className="text-3xl">{t("colorsGame")}</CardTitle>
              <CardDescription className="text-lg">{t("colorsDesc")}</CardDescription>
            </CardHeader>
            <CardContent className="text-center">
              <Button
                onClick={() => setShowPlayScreen(true)}
                size="lg"
                className="bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-white px-8 py-4 text-xl"
              >
                {t("playNow")}
              </Button>
            </CardContent>
