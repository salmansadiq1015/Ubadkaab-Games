...
import { Calculator, Trophy, Star, Target, ArrowLeft, Globe } from "lucide-react"
import MathPlayScreen from "./math-play"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"

interface MathMainPageProps {
  onBack: () => void
}

export default function MathMainPage({ onBack }: MathMainPageProps) {
  const { t, language, setLanguage } = useLanguage()
  const { progress } = useProgress()
  const [showPlayScreen, setShowPlayScreen] = useState(false)

  if (showPlayScreen) {
    return <MathPlayScreen onBack={() => setShowPlayScreen(false)} />
  }

  const gameStats = {
    highScore: progress.bestScores.math || 0,
        ...
        <div className="max-w-4xl mx-auto">
          <Card className="bg-white/60 backdrop-blur-sm border-white/20 mb-8">
            <CardHeader className="text-center">
              <div className="w-24 h-24 bg-gradient-to-br from-blue-400 to-blue-600 rounded-3xl flex items-center justify-center mx-auto mb-4">
                <Calculator className="w-12 h-12 text-white" />
              </div>
              <CardTitle className="text-3xl">{t("mathGame")}</CardTitle>
              <CardDescription className="text-lg">{t("mathDesc")}</CardDescription>
            </CardHeader>
            <CardContent className="text-center">
              <Button
                onClick={() => setShowPlayScreen(true)}
                size="lg"
                className="bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white px-8 py-4 text-xl"
              >
                {t("playNow")}
              </Button>
            </CardContent>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <span>Learn basic addition</span>
                </div>
                <div className="flex items-center gap-3">
                  <Badge variant="secondary">Subtraction</Badge>
                  <span>Practice subtraction</span>
                </div>
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span>Easy mobile controls</span>
                </div>
              </div>
            </CardContent>
          </Card>
