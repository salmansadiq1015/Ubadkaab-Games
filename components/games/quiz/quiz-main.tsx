...
import { Brain, Trophy, Star, Target, ArrowLeft, Globe } from "lucide-react"
import QuizPlayScreen from "./quiz-play"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"

interface QuizMainPageProps {
  onBack: () => void
}

export default function QuizMainPage({ onBack }: QuizMainPageProps) {
  const { t, language, setLanguage } = useLanguage()
  const { progress } = useProgress()
  const [showPlayScreen, setShowPlayScreen] = useState(false)

  if (showPlayScreen) {
    return <QuizPlayScreen onBack={() => setShowPlayScreen(false)} />
  }

  const gameStats = {
    highScore: progress.bestScores.quiz || 0,
        ...
        <div className="max-w-4xl mx-auto">
          <Card className="bg-white/60 backdrop-blur-sm border-white/20 mb-8">
            <CardHeader className="text-center">
              <div className="w-24 h-24 bg-gradient-to-br from-orange-400 to-orange-600 rounded-3xl flex items-center justify-center mx-auto mb-4">
                <Brain className="w-12 h-12 text-white" />
              </div>
              <CardTitle className="text-3xl">{t("quizGame")}</CardTitle>
            ...
            <CardContent className="text-center">
              <Button
                ...
                size="lg"
                className="bg-gradient-to-r from-orange-500 to-yellow-500 hover:from-orange-600 hover:to-yellow-600 text-white px-8 py-4 text-xl"
              >
                {t("playNow")}
              </Button>
            </CardContent>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <span>Fun facts and trivia</span>
                </div>
                <div className="flex items-center gap-3">
                  <Badge variant="secondary">Multiple Choice</Badge>
                  <span>Easy answer selection</span>
                </div>
                <div className="flex items-center gap-3">
