...
import { Hash, Trophy, Star, Target, ArrowLeft, Globe } from "lucide-react"
import NumbersPlayScreen from "./numbers-play"

interface NumbersMainPageProps {
  onBack: () => void
}

export default function NumbersMainPage({ onBack }: NumbersMainPageProps) {
  const { t, language, setLanguage } = useLanguage()
  const { progress } = useProgress()
  const [showPlayScreen, setShowPlayScreen] = useState(false)

  if (showPlayScreen) {
    return <NumbersPlayScreen onBack={() => setShowPlayScreen(false)} />
  }

  const gameStats = {
    highScore: progress.bestScores.numbers || 0,
        ...
        <div className="max-w-4xl mx-auto">
          <Card className="bg-white/60 backdrop-blur-sm border-white/20 mb-8">
            <CardHeader className="text-center">
              <div className="w-24 h-24 bg-gradient-to-br from-cyan-400 to-cyan-600 rounded-3xl flex items-center justify-center mx-auto mb-4">
                <Hash className="w-12 h-12 text-white" />
              </div>
              <CardTitle className="text-3xl">{t("numbersGame")}</CardTitle>
              <CardDescription className="text-lg">{t("numbersDesc")}</CardDescription>
            </CardHeader>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <span>Learn to count objects</span>
                </div>
                <div className="flex items-center gap-3">
                  <Badge variant="secondary">Recognition</Badge>
                  <span>Number identification</span>
                </div>
                <div className="flex items-center gap-3">
                  <Badge variant="secondary">Visual</Badge>
                  <span>Colorful objects to count</span>
                </div>
                <div className="flex items-center gap-3">
