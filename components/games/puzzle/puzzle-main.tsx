...
import { Puzzle, Trophy, Star, Target, ArrowLeft, Globe } from "lucide-react"
import PuzzlePlayScreen from "./puzzle-play"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"

interface PuzzleMainPageProps {
  onBack: () => void
}

export default function PuzzleMainPage({ onBack }: PuzzleMainPageProps) {
  const { t, language, setLanguage } = useLanguage()
  const { progress } = useProgress()
  const [showPlayScreen, setShowPlayScreen] = useState(false)

  if (showPlayScreen) {
    return <PuzzlePlayScreen onBack={() => setShowPlayScreen(false)} />
  }

  const gameStats = {
    highScore: progress.bestScores.puzzle || 0,
        ...
        <div className="max-w-4xl mx-auto">
          <Card className="bg-white/60 backdrop-blur-sm border-white/20 mb-8">
            <CardHeader className="text-center">
              <div className="w-24 h-24 bg-gradient-to-br from-purple-400 to-purple-600 rounded-3xl flex items-center justify-center mx-auto mb-4">
                <Puzzle className="w-12 h-12 text-white" />
              </div>
              <CardTitle className="text-3xl">{t("puzzleGame")}</CardTitle>
              <CardDescription className="text-lg">{t("puzzleDesc")}</CardDescription>
            </CardHeader>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span>Recognize number patterns</span>
                </div>
                <div className="flex items-center gap-3">
                  <Badge variant="secondary">Strategy</Badge>
                  <span>Plan your moves</span>
                </div>
                <div className="flex items-center gap-3">
                  <Badge variant="secondary">Touch</Badge>
                  <span>Drag and drop controls</span>
                </div>
              </div>
            </CardContent>
          </Card>
