...
import { BookOpen, Trophy, Star, Target, ArrowLeft, Globe } from "lucide-react"
import AlphabetPlayScreen from "./alphabet-play"

interface AlphabetMainPageProps {
  onBack: () => void
}

export default function AlphabetMainPage({ onBack }: AlphabetMainPageProps) {
  const { t, language, setLanguage } = useLanguage()
  const { progress } = useProgress()
  const [showPlayScreen, setShowPlayScreen] = useState(false)

  if (showPlayScreen) {
    return <AlphabetPlayScreen onBack={() => setShowPlayScreen(false)} />
  }

  const gameStats = {
    highScore: progress.bestScores.alphabet || 0,
    timesPlayed: Math.floor(progress.gamesPlayed / 6) || 0,
    accuracy: "85%", // This could be calculated from actual game data
  }
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <Card className="bg-white/60 backdrop-blur-sm border-white/20 mb-8">
            <CardHeader className="text-center">
              <div className="w-24 h-24 bg-gradient-to-br from-pink-400 to-pink-600 rounded-3xl flex items-center justify-center mx-auto mb-4">
                <BookOpen className="w-12 h-12 text-white" />
              </div>
              <CardTitle className="text-3xl">{t("alphabetGame")}</CardTitle>
              <CardDescription className="text-lg">{t("alphabetDesc")}</CardDescription>
            </CardHeader>
            <CardContent className="text-center">
              <Button
                onClick={() => setShowPlayScreen(true)}
                size="lg"
                className="bg-gradient-to-r from-pink-500 to-purple-500 hover:from-pink-600 hover:to-purple-600 text-white px-8 py-4 text-xl"
              >
                {t("playNow")}
              </Button>
            </CardContent>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span>Learn letters and sounds</span>
                </div>
                <div className="flex items-center gap-3">
                  <Badge variant="secondary">Progressive</Badge>
                  <span>Difficulty increases with level</span>
                </div>
                <div className="flex items-center gap-3">
