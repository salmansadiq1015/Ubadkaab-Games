...
import { PawPrint, Trophy, Star, Target, ArrowLeft, Globe } from "lucide-react"
import AnimalsPlayScreen from "./animals-play"

interface AnimalsMainPageProps {
  onBack: () => void
}

export default function AnimalsMainPage({ onBack }: AnimalsMainPageProps) {
  const { t, language, setLanguage } = useLanguage()
  const { progress } = useProgress()
  const [showPlayScreen, setShowPlayScreen] = useState(false)

  if (showPlayScreen) {
    return <AnimalsPlayScreen onBack={() => setShowPlayScreen(false)} />
  }

  const gameStats = {
    highScore: progress.bestScores.animals || 0,
        ...
        <div className="max-w-4xl mx-auto">
          <Card className="bg-white/60 backdrop-blur-sm border-white/20 mb-8">
            <CardHeader className="text-center">
              <div className="w-24 h-24 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-3xl flex items-center justify-center mx-auto mb-4">
                <PawPrint className="w-12 h-12 text-white" />
              </div>
              <CardTitle className="text-3xl">{t("animalsGame")}</CardTitle>
              <CardDescription className="text-lg">{t("animalsDesc")}</CardDescription>
            </CardHeader>
            <CardContent className="text-center">
              <Button
                onClick={() => setShowPlayScreen(true)}
                size="lg"
                className="bg-gradient-to-r from-emerald-500 to-green-500 hover:from-emerald-600 hover:to-green-600 text-white px-8 py-4 text-xl"
              >
                {t("playNow")}
              </Button>
            </CardContent>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span>Animal sound recognition</span>
                </div>
                <div className="flex items-center gap-3">
                  <Badge variant="secondary">Habitats</Badge>
                  <span>Where animals live</span>
                </div>
                <div className="flex items-center gap-3">
                  <Badge variant="secondary">Fun Facts</Badge>
                  <span>Amazing animal trivia</span>
                </div>
              </div>
            </CardContent>
          </Card>
