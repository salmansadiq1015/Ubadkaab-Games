...
import { ArrowLeft, RotateCcw, Plus, Minus, X, Divide, Target } from "lucide-react"
...
export default function MathGame() {
  ...
  const generateQuestion = (operations: Operation[], maxNum: number, level: number): Question => {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      if (questionsAnswered + 1 >= levelConfig.questionCount) {
        ...
        setQuestion(generateQuestion(levelConfig.operations, levelConfig.maxNumber, level))
      }
    }, 1500)
  }
  ...
  const handleLevelComplete = () => {
    ...
    if (accuracy >= 0.7) {
      playSound("correct")
      speakText(`Level ${level} complete! Great job with ${Math.round(accuracy * 100)}% accuracy!`)
    } else {
      speakText(`Level ${level} complete! Keep practicing to improve your accuracy!`)
    }
  }
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center space-x-4">
            ...
            {timeLeft !== null && (
              <div
                className={`rounded-full px-4 py-2 shadow-lg ${timeLeft <= 10 ? "bg-red-100 text-red-800" : "bg-white text-gray-800"}`}
              >
        ...
        <div className="text-center mb-8">
          ...
          <div className="bg-white rounded-xl p-4 shadow-lg max-w-2xl mx-auto">
            <h2 className="text-xl font-bold text-gray-800 mb-2">Level {level} Challenge</h2>
            ...
            <p className="text-gray-600">
              Questions: {questionsAnswered}/{levelConfig.questionCount} | Correct: {correctAnswers} | Max Number:{" "}
              {levelConfig.maxNumber}
            </p>
            <div className="mt-2">
              <div className="bg-gray-200 rounded-full h-3 relative overflow-hidden">
                <div
                  className="bg-gradient-to-r from-yellow-500 to-orange-500 h-full rounded-full transition-all duration-500"
                  style={{ width: `${(questionsAnswered / levelConfig.questionCount) * 100}%` }}
                />
              </div>
            </div>
          </div>
            ...
            <Card className="bg-white shadow-2xl max-w-2xl mx-auto">
              <CardContent className="p-8">
                ...
                <div className="space-y-2 text-lg text-gray-700 mb-6">
                  ...
                  <p>Accuracy: {Math.round((correctAnswers / Math.max(questionsAnswered, 1)) * 100)}%</p>
