...
export default function AlphabetsGame() {
  ...
  const [currentChallenge, setCurrentChallenge] =
    useState<LevelChallenge | null>(null);
  const [challengeProgress, setChallengeProgress] = useState<Set<string>>(
    new Set()
  );
  ...
  const generateLevelChallenge = (level: number): LevelChallenge => {
    ...
    if (level <= 2) {
      ...
      const targetLetters = shuffledAlphabet.slice(
        0,
        Math.min(5 + level * 2, letters.length)
      );
      return {
        type: "letters",
      ...
      // Simple words
      const levelWords = simpleWords.filter(
        (word) => word.length <= Math.floor(level / 2) + 2
      );
      const targetWords = levelWords
        .sort(() => Math.random() - 0.5)
        .slice(0, 5);
      return {
        type: "words",
      ...
      const targetWords = complexWords
        .sort(() => Math.random() - 0.5)
        .slice(0, Math.min(level - 10, 8));
      return {
        type: "phonics",
        target: targetWords,
  ...
  const handleWordComplete = () => {
    ...
    const currentWord = currentChallenge.target.find(
      (word) => word === userInput && !challengeProgress.has(word)
    );
    ...
    if (currentWord) {
      ...
      if (language === "ar") {
        speakArabic(
          `أحسنت! لقد هجيت ${currentWord} بشكل صحيح!`,
          `Excellent! You spelled ${currentWord} correctly!`
        );
      } else if (language === "so") {
  ...
  const handleSpellingSubmit = () => {
    ...
    const currentWord = currentChallenge.target.find(
      (word) =>
        word.toLowerCase() === userInput.toLowerCase() &&
  ...
  const handleNextLevel = () => {
    if (challengeProgress.size >= currentChallenge?.target.length!) {
      ...
      if (language === "ar") {
        speakArabic(
          `المستوى ${level + 1} مفتوح! عمل رائع!`,
          `Level ${level + 1} unlocked! Great job!`
        );
      } else if (language === "so") {
  ...
  const isLevelComplete =
    currentChallenge &&
    challengeProgress.size >= currentChallenge.target.length;
              ...
              <div className="mt-2">
                <div className="bg-gray-200 rounded-full h-3 relative overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${
                        (challengeProgress.size /
                          currentChallenge.target.length) *
                        100
                      }%`,
                    }}
                  />
                ...
                <p className="text-sm text-gray-600 mt-1">
                  {t("progress")}: {challengeProgress.size}/
                  {currentChallenge.target.length}
                </p>
              </div>
            </div>
                    ...
                    !challengeProgress.has(word) && (
                      <div key={word} className="mb-4">
                        ...
                        <input
                          ...
                          className={`w-full p-2 border-2 border-gray-300 rounded-lg text-center text-lg font-bold ${
        ...
        {selectedLetter && (
          <div className="text-center mb-8">
            <div className="inline-block bg-white rounded-3xl shadow-2xl p-8 animate-bounce">
              <span
                className={`text-8xl font-bold text-red-500 mb-4 block ${
                  language === "ar" ? "font-arabic" : ""
                }`}
              >
                {selectedLetter}
